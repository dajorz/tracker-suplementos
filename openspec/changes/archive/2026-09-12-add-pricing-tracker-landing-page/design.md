...
- **GA4 placeholder ID** (`G-XXXXXXXXXX`): requires a property the user owns; the code ships wired up and inert until substituted. The contact address (deliberately not written out here; it is assembled at runtime from fragments in `index.html`) and canonical URL (`https://dajorz.github.io/tracker-suplementos/`) are known and hardcoded.
