...
- Neutralizar el tinte del disclaimer de precios (ámbar → `slate`) y retirarle el icono ⚠, de modo que la tira de Telegram sea el único bloque con color entre la cabecera y la tabla. El texto legal, la posición y la imposibilidad de ocultarlo no cambian.
...
- `pricing-tracker-landing-page`: se añade un requisito de llamada a la acción hacia el canal de Telegram y otro de accesibilidad de texto y controles; el requisito de orden del contenido principal pasa a admitir una única tira slim sobre la tabla; el requisito de la CTA de sugerencias acota su prohibición de alertas a su propia sección; el requisito del disclaimer de precios pasa a exigir tinte neutro y sin icono de aviso; el requisito de retirada de consentimiento sustituye «texto de bajo contraste» por «texto atenuado pero conforme a AA».

## Impact

- Ficheros afectados: `index.html` únicamente (un bloque nuevo entre el disclaimer y `<main>`).
- Coste vertical: 48px medidos. El borde superior de la tabla debe seguir siendo visible en un viewport de 900px de alto; es el escenario que este cambio puede romper y el que hay que verificar.
